...
function thewire_tools_access_write_hook($hook_name, $entity_type, $return, $params){
	if((elgg_in_context("thewire")|| elgg_in_context("activity")) && ($user = elgg_get_logged_in_user_entity())){
		if(is_array($return)){
			unset($return[ACCESS_PRIVATE]);
			unset($return[ACCESS_LOGGED_IN]);
      //unset($return[ACCESS_FRIENDS]);
			
      $return[ACCESS_FRIENDS]=elgg_echo('canaccess:friends');
      $return[ACCESS_PUBLIC]=elgg_echo('canaccess:public');
			$options = array(
				"type" => "group",
				"limit" => false,
				"relationship" => "member",
			...
			if($groups = elgg_get_entities_from_relationship($options)){
				foreach($groups as $group){
					if($group->thewire_enable !== "no"){
						$return[$group->group_acl] = elgg_echo('canaccess:groups').":".$group->name;
					}
				}
			}
		}		
...
function thewire_tools_widget_title_url($hook_name, $entity_type, $return, $params){
	...
	if(empty($result) && ($widget instanceof ElggWidget)){
		switch($widget->handler) {
			case "thewire":
				$result = "/thewire/owner/" . $widget->getOwnerEntity()->name;//username;
				break;
			case "index_thewire":
			case "thewire_post":
				$result = "/thewire/all";
