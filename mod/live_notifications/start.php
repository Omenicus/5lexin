...
function create_message_for_entity($to_entity, $from_entity, $type, $action_type, $entity, $annotation=NULL){
	...
	if($action_type=='comment'){
		$tags = live_notifications_tag_user($annotation->value, $entity, $from_entity);
		analize_thread_comments($entity, $annotation, $from_entity, $tags);
		if($from_entity->guid!=$entity->owner_guid){
			...
			$description =  elgg_echo('live_notifications:comments:create', array($url_user, $to_entity->getUrl()));
	...
	if($entity->getSubtype()=='thewire' && $action_type!='comment'){
		...
		if($entity->wire_thread != $entity->guid){
			...
			if($thread->owner_guid!=$from_entity->guid && !in_array($thread->owner_guid, $tags)){
				$description =  elgg_echo('live_notifications:thewire:reply', array($url_user, $CONFIG->wwwroot.'thewire/thread/'.$thread->guid));		
				$description .= '<br/><i>'.elgg_get_excerpt($entity->description,60).'</i>';
...
function analize_thread_comments($entity, $annotation, $from_entity, $tags){
    ...
    if ($comments) {       
        foreach ($comments as $comment) {
            ...
            if($owner_guid!=$from_entity->guid && $owner_guid!=$entity->owner_guid && !in_array($owner_guid, $autors_comments) && !in_array($owner_guid, $tags)){
            	$url_user = elgg_view('output/url', array(
					'href' => $from_entity->getURL(),
					'text' => $from_entity->name,
					'class' => 'elgg-river-subject',
				));            	
            	$description =  elgg_echo('live_notifications:comments:thread', array($url_user, $entity->getUrl()));
				$description .= '<a href="'.$entity->getUrl().'" title="">'.$entity->title.'</a> <br/>';
				$description .= '<i>'.elgg_get_excerpt($annotation->value,50).'</i>';
...
function get_user_by_name_1($username) {
...
function live_notifications_tag_user($value, $entity, $from_user){
    ...
    foreach($users_tagging as $i=>$username){
        ...
        $user_tag = get_user_by_name_1($username);
        if (!$user_tag)
            $user_tag=get_user_by_name($username);
