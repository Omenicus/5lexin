...
function checkMessages() { 
	...
	$.ajax({
		url : "<?php echo $site_url; ?>mod/chat/chat.php?action=rx", 
	...
	function checkFriend(){
   		$.ajax({
			url : "<?php echo $site_url; ?>mod/chat/chat.php?action=cf", 
    ...
    function checkChatBoxInputKey(event, chatboxtextarea, chatboxtitle) {
        ...
        if(event.keyCode == 13 && event.shiftKey == 0)  {
			...
			if (message != '') {
                $.ajax({
					url : "<?php echo $site_url; ?>mod/chat/chat.php?action=tx", 
					data : {from: '<?php echo $_SESSION['user']->guid; ?>' ,to: chatboxtitle, message: origmsg}, 
					success: function(data,status){
						try{
							var response = JSON.parse(data);
							message = response.message.replace(/^\s+|\s+$/g,"");
							message = bbParseText(message.replace(/^\s+|\s+$/g,"").replace(/\n/g,'<br />'));
	...
	function bbParseText(text) {
		...
		text = text.replace(/((https?\:\/\/|ftp\:\/\/)|(www\.))(\S+)(\w{2,4})(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/gi,function(url){
			...
			if ( url.indexOf('<?php echo substr($site_url,7); ?>') != -1){
		...
		for (var i = 0; i < smiley_array.length; i++) {
			var smiley_img = '<img alt="" src="<?php echo $site_url; ?>mod/chat/graphics/icons/smilies/' + smiley_xhtml[i] + '" />';
